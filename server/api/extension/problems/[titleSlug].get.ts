...
import { db } from "../../../db";
import { leetcodeQuestions, problems, reviews } from "../../../db/schema";
...
export default defineEventHandler(async (event): Promise<ExtensionProblemResponse> => {
  ...
  const [question] = await db.select().from(leetcodeQuestions).where(eq(leetcodeQuestions.titleSlug, titleSlug)).limit(1);
  if (!question) {
