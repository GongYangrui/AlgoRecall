...
class RecallPanel {
  ...
  async changePage(slug: string | null) {
  ...
  private async loadConnectedPage() {
    if (this.slug) {
  ...
  private async loadProblem() {
    ...
    this.render();
    try {
      const problem = await send<ExtensionProblemResponse>({ type: "GET_PROBLEM", titleSlug });
      if (this.slug !== titleSlug) return;
      ...
      if (this.slug === titleSlug) this.handleError(error);
    }
  }
  ...
  private title() {
    if (!this.slug) {
      const site = getLeetcodeSiteLabel(location.href);
      return `<div class="ar-title" title="${escapeHtml(site)}">${escapeHtml(site)}</div>`;
    }
    const question = this.problem?.question;
    if (!question) {
      const fallback = `LeetCode · ${this.slug}`;
      return `<div class="ar-title" title="${escapeHtml(fallback)}">${escapeHtml(fallback)}</div>`;
    }
    const title = `${question.questionFrontendId}. ${formatProblemTitle(question)}`;
    return `<div class="ar-title" title="${escapeHtml(title)}">${escapeHtml(title)}</div>`;
  }
  ...
  private body() {
    if (this.state === "checking") {
      return this.slug
        ? this.status("…", "正在读取题目", "同步 AlgoRecall 中的记录状态。", false, true)
        : this.status("…", "正在读取 AlgoRecall", "同步账号连接和今日复习。", false, true);
    }
    ...
    if (this.state === "disconnected" || this.state === "auth-required") {
      return `<p class="ar-kicker">30 天独立连接</p><h2 class="ar-h2">${this.state === "auth-required" ? "连接已失效" : "连接 AlgoRecall"}</h2><p class="ar-copy">连接后可在任意 LeetCode 页面查看今日复习，并在题目页直接记录。扩展无法访问你的网站 Cookie 或编辑器内容。</p><div class="ar-actions"><button class="ar-btn ar-btn-primary" data-action="connect">连接账号</button></div><div class="ar-meta">仅授予题目查询和复习记录权限</div>`;
    }
    if (!this.connected) return this.recordView();
    if (!this.slug) return this.homeView();
  ...
  private homeView() {
    if (this.queueState === "idle" || this.queueState === "loading") {
      return this.status("…", "正在读取今日复习", "同步网页端的待复习队列。", false, true);
    }
    if (this.queueState === "offline" || this.queueState === "error") return this.queueErrorView();
    if (this.todayProblems.length === 0) return this.emptyQueueView();
    const count = this.todayProblems.length;
    return `<div class="ar-home-head"><div><p class="ar-kicker">今日计划</p><h2 class="ar-h2">还有 ${count} 题待复习</h2></div><span class="ar-queue-count">${count} 题</span></div><p class="ar-copy">从队列第一题开始，或直接选择下面任意题目。</p><div class="ar-actions"><button class="ar-btn ar-btn-primary" data-action="start">开始复习</button></div><div class="ar-home-queue">${this.queueList("选择题目", false)}</div>`;
  }

  private queueErrorView() {
  ...
  private queueList(heading = "今日待复习", showCount = true) {
    const items = this.todayProblems.map((problem) => {
      const current = Boolean(this.slug && problem.titleSlug === this.slug);
      const title = formatProblemTitle(problem);
      const number = `#${problem.frontendId || "--"}`;
      const accessibleTitle = `${number} · ${title}`;
      return `<li><button class="ar-queue-item${current ? " current" : ""}" data-slug="${escapeHtml(problem.titleSlug)}" aria-label="${escapeHtml(`${current ? "当前题目" : "打开题目"} ${accessibleTitle}`)}" ${current ? "disabled" : ""}><div><div class="ar-queue-title"><span class="ar-number">${escapeHtml(number)}</span><div class="ar-queue-title-text" title="${escapeHtml(title)}">${escapeHtml(title)}</div>${current ? '<span class="ar-current-badge">当前</span>' : ""}</div><div class="ar-queue-meta">${escapeHtml(difficultyLabel(problem.difficulty))} · ${escapeHtml(statusLabel(problem.status))}</div></div><span class="ar-queue-stage"><strong>第 ${problem.stage} 阶段</strong>复习 ${problem.reviewCount} 次</span></button></li>`;
    }).join("");
    return `<div class="ar-queue-head"><strong>${escapeHtml(heading)}</strong>${showCount ? `<span class="ar-queue-count">${this.todayProblems.length} 题</span>` : ""}</div><ul class="ar-queue">${items}</ul><div class="ar-queue-footer"><button class="ar-link-btn" data-action="manage">管理扩展连接</button></div>`;
  }
  ...
  private bind() {
    ...
    this.root.querySelector<HTMLElement>("[data-action='start']")?.addEventListener("click", () => this.goNext());
  ...
  private async submit(result: ReviewResult | null = this.selected) {
    const titleSlug = this.slug;
    if (!titleSlug || !result || this.state === "submitting") return;
...
function bootstrap() {
  ...
  const panel = new RecallPanel(slug);
  ...
  const checkUrl = () => {
    ...
    void panel.changePage(nextSlug);
  };
