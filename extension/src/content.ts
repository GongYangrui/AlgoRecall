...
.ar-results{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:12px}.ar-result{--ar-tone:var(--ar-primary);min-height:43px;padding:8px 9px;border:1px solid color-mix(in srgb,var(--ar-tone) 35%,var(--ar-border));border-radius:9px;background:color-mix(in srgb,var(--ar-tone) 10%,var(--ar-bg));color:var(--ar-text);font-size:12px;font-weight:800;text-align:center;cursor:pointer}.ar-result[data-tone="error"]{--ar-tone:var(--ar-error)}.ar-result[data-tone="info"]{--ar-tone:var(--ar-info)}.ar-result[data-tone="success"]{--ar-tone:var(--ar-success)}.ar-result:hover{background:color-mix(in srgb,var(--ar-tone) 16%,var(--ar-bg))}.ar-result[aria-pressed="true"]{border-color:var(--ar-tone);background:color-mix(in srgb,var(--ar-tone) 18%,var(--ar-bg));box-shadow:inset 0 0 0 1px var(--ar-tone)}.ar-result:disabled{cursor:not-allowed;opacity:.55}
...
class RecallPanel {
  ...
  private recordView() {
    ...
    return `${this.progress()}<p class="ar-kicker">记录一次复习</p><h2 class="ar-h2">这道题做得怎么样？</h2><div class="ar-results">${RESULT_OPTIONS.map((result) => `<button class="ar-result" data-result="${result}" data-tone="${reviewResultTone(result)}" aria-pressed="${this.selected === result}" ${this.state === "submitting" ? "disabled" : ""}>${escapeHtml(resultLabel(result))}</button>`).join("")}</div><label class="ar-label" for="ar-note"><span>本次备注，可不填</span><span class="ar-count">${this.note.length}/${REVIEW_NOTE_MAX_LENGTH}</span></label><textarea id="ar-note" class="ar-note" maxlength="${REVIEW_NOTE_MAX_LENGTH}" placeholder="记录这次刷题的收获" ${this.state === "submitting" ? "disabled" : ""}>${escapeHtml(this.note)}</textarea><div class="ar-actions"><button class="ar-btn ar-btn-primary" data-action="submit" ${!this.selected || this.state === "submitting" ? "disabled" : ""}>${this.state === "submitting" ? '<span class="ar-spinner"></span>正在记录' : "记录到 AlgoRecall"}</button></div>`;
  }
  ...
  private todayView() {
    ...
    const items = this.todayProblems.map((problem) => {
      ...
      return `<li><button class="ar-queue-item${current ? " current" : ""}" data-slug="${escapeHtml(problem.titleSlug)}" ${current ? "disabled" : ""}><div><div class="ar-queue-title"><span class="ar-number">#${escapeHtml(problem.frontendId || "--")}</span><strong>${escapeHtml(problem.titleCn || problem.title)}</strong>${current ? '<span class="ar-current-badge">当前</span>' : ""}</div><div class="ar-queue-meta">${escapeHtml(difficultyLabel(problem.difficulty))} · ${escapeHtml(statusLabel(problem.status))}</div></div><span class="ar-queue-stage"><strong>第 ${problem.stage} 阶段</strong>复习 ${problem.reviewCount} 次</span></button></li>`;
    }).join("");
